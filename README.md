...
## 2.0 Решение:
Есть несколько способов добавить программу в автозапуск:

1. Папка автозагрузки. Так как мы имеем доступ к компьютеру злоумышленника через удаленный стол, мы можем добавить нашу программу в папку атозагрузки, например, через **win + r**, ввести **shell:startup**, или перейти в папку сразу по адресу **C:\Users\UserName\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup**
и добавить туда ярлык на нашу программу.

2. Запись к ключам запуска в реестре Windows. Win + R -> regedit -> HKEY-LOCAL_MACHINE -> Кликаем правой кнопкой мыши в основном окне -> Добавить строковый параметр -> Указываем имя и путь до exe-файла.

3. Запланированные задачи.
Открываем планировшик заданий -> создать задачу -> Настраиваем время запуска программы -> Запустить программу -> Путь до exe-файла -> Готово.
